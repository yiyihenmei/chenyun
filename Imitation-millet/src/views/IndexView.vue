<template>
  <div id="index">
    <header>
      <div class="w clearfix">
        <div class="fr">
          <span v-show="!Boolean(userName)">
            <a @click="login">登录</a>|
            <a @click="register">注册</a>
          </span>
          <span v-show="Boolean(userName)" @dblclick="signout">
            欢迎&nbsp;{{ userName }}
          </span>
          <span @click="goorder">我的订单</span>
          <span @click="mylike">我的收藏</span>
          <el-button
          :class="flag?'color':''"
            size="large"
            @click="goCart"
            color="#424242"
            :dark="isDark"
          >
            <el-icon><ShoppingCartFull /></el-icon>
            购物车(<span>{{ store.state.num }}</span
            >)
          </el-button>
        </div>
      </div>
    </header>
    <div class="header-1">
      <div class="w clearfix">
        <img src="http://101.132.181.9/img/logo.c8999c02.png" class="fl" />
        <ul class="tabs">
          <li v-for="(ele, i) in tablist" :key="i" @click="tabswitch(i)">
            <router-link :to="ele.path">
              {{ ele.name }}
            </router-link>
          </li>
        </ul>
        <div class="box fr">
          <el-input
            v-model="input"
            placeholder="请输入搜索内容"
            style="width: 300px; height: 42px"
          >
            <template #append @click="search">
              <el-icon @click="search"><Search /></el-icon>
            </template>
          </el-input>
        </div>
      </div>
    </div>
    <div class="section">
      <router-view> </router-view>
    </div>
    <footer>
      <div class="footer_top">
        <div class="w">
          <ul>
            <li>
              <a href="javasccript:;"> 7天无理由退换货 </a>
            </li>
            <li>
              <a href="javasccript:;"> 满99元全场免邮 </a>
            </li>
            <li>
              <a href="javasccript:;"> 100%品质保证 </a>
            </li>
          </ul>
        </div>
      </div>
      <div class="footer_bottom">
        <div class="w">
          <div class="icon"></div>
          <p class="footer_tabs">
            <router-link to="/home">首页</router-link>
            <span>|</span>
            <router-link to="/goods">全部商品</router-link>
            <span>|</span>
            <router-link to="/about">关于我们</router-link>
          </p>
          <p class="footer_text">商城版权所有 © 2012-2021</p>
        </div>
      </div>
    </footer>

    <!-- 登录弹出框 -->
    <el-dialog
      v-model="dialogVisible"
      title="登录"
      width="25%"
      :before-close="handleClose"
    >
      <span>
        <p class="ipt">
          <span>
            <el-input
              size="large"
              v-model="username"
              class="w-50 m-2"
              placeholder="请填写用户名"
              :prefix-icon="UserFilled"
            />
          </span>
        </p>
        <p class="ipt">
          <span>
            <el-input
              size="large"
              type="password"
              v-model="password"
              class="w-50 m-2"
              placeholder="请填写密码"
              :prefix-icon="View"
            />
          </span>
        </p>
      </span>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="dialogVisible = false">取消</el-button>
          <el-button type="primary" @click="gologin">确定</el-button>
        </span>
      </template>
    </el-dialog>

    <!-- 注册弹出框 -->
    <el-dialog
      v-model="dialogVisible1"
      title="注册"
      width="25%"
      :before-close="handleClose1"
    >
      <span>
        <p class="ipt">
          <span>
            <el-input
              size="large"
              v-model="username1"
              class="w-50 m-2"
              placeholder="请填写用户名"
              :prefix-icon="UserFilled"
            />
          </span>
        </p>
        <p class="ipt">
          <span>
            <el-input
              size="large"
              type="password"
              v-model="password1"
              class="w-50 m-2"
              placeholder="请填写密码"
              :prefix-icon="View"
            />
          </span>
        </p>
        <p class="ipt">
          <span>
            <el-input
              size="large"
              type="password"
              v-model="confirmPd"
              class="w-50 m-2"
              placeholder="请再次确认密码"
              :prefix-icon="View"
            />
          </span>
        </p>
      </span>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="dialogVisible1 = false">取消</el-button>
          <el-button type="primary" @click="getregister">确定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
</template>
<script setup>
import { goLogin, getRegister } from "@/api/Interface/home.js";
import { ref, reactive, onMounted,watch } from "vue";
import { useRouter } from "vue-router";
import { ElMessageBox, ElMessage } from "element-plus";
import { UserFilled, View } from "@element-plus/icons-vue";
import { useStore } from "vuex";

const router = useRouter();
const store = useStore();

let input = ref("");
const flag=ref(false)
// 登录
const dialogVisible = ref(false);
const username = ref("");
const password = ref("");

const userName = ref("");
// 注册
const dialogVisible1 = ref(false);
const username1 = ref("");
const password1 = ref("");
const confirmPd = ref("");

const tablist = ref([
  {
    name: "首页",
    path: "/home",
  },
  {
    name: "全部商品",
    path: "/goods",
  },
  {
    name: "关于我们",
    path: "/about",
  },
]);
const bannerimg =
  ref[
    [
      "http://101.132.181.9:3000/public/imgs/cms_1.jpg",
      "http://101.132.181.9:3000/public/imgs/cms_2.jpg",
      "http://101.132.181.9:3000/public/imgs/cms_3.jpg",
      "http://101.132.181.9:3000/public/imgs/cms_4.jpg",
    ]
  ];
const tabindex = ref(0);

onMounted(() => {
  if (JSON.parse(localStorage.getItem("user"))) {
    userName.value = JSON.parse(localStorage.getItem("user")).userName;
  }
});
const tabswitch = (i) => {
  tabindex.value = i;
};
// 登录
const login = () => {
  dialogVisible.value = true;
};
const gologin = async () => {
  let data = await goLogin(username.value, password.value);
  if (data.code == "001") {
    ElMessage({
      message: "登录成功",
      type: "success",
    });
    localStorage.setItem("user", JSON.stringify(data.user));
    userName.value = JSON.parse(localStorage.getItem("user")).userName;
  } else {
    ElMessage({
      message: data.msg,
      type: "error",
    });
  }
  dialogVisible.value = false;
};
// 注册
const register = () => {
  dialogVisible1.value = true;
};
const getregister = async () => {
  let data = await getRegister(username1.value, password1.value);
  if (data.code == "001") {
    ElMessage({
      message: "注册成功",
      type: "success",
    });
  } else {
    ElMessage({
      message: data.msg,
      type: "error",
    });
  }
  dialogVisible1.value = false;
};
// 退出
const signout = () => {
  ElMessageBox.confirm("是否要退出登录?")
    .then(() => {
      userName.value = "";
      localStorage.removeItem("user");
    })
    .catch(() => {});
};
// 我的订单
const goorder = () => {
  if (JSON.parse(localStorage.getItem("user"))) {
    router.push("/myorder");
  } else {
    dialogVisible.value = true;
  }
};
// 我的收藏
const mylike = () => {
  if (JSON.parse(localStorage.getItem("user"))) {
    router.push("/collection");
  } else {
    dialogVisible.value = true;
  }
};
// 购物车
const goCart = () => {
  if (JSON.parse(localStorage.getItem("user"))) {
    router.push("/shoppingCart");
  } else {
    dialogVisible.value = true;
  }
};

const search = () => {
  router.push("/goods?search=" + input.value);
};

const handleClose = () => {
  ElMessageBox.confirm("是否要退出登录?")
    .then(() => {
      dialogVisible.value = false;
    })
    .catch(() => {});
};
const handleClose1 = () => {
  ElMessageBox.confirm("是否要退出注册?")
    .then(() => {
      dialogVisible1.value = false;
    })
    .catch(() => {});
};

watch(
  () => store.state.num,
  (val) => {
    if (val>0) {
      flag.value=true
    }else{
      flag.value=false
    }
  },
  {
    deep: true,
  }
);
</script>

<style lang="scss" scoped>
#index {
  header {
    background-color: #3d3d3d;
    margin-bottom: 20px;
    .w {
      .fr {
        color: #b0b0b0;
        font-size: 14px;
        span {
          margin-right: 10px;
          a {
            color: #b0b0b0;
          }
        }
        .el-button {
          color: #b0b0b0;
        }
        .color{
          background-color: #ed712e !important;
          color: #fff !important;
        }
      }
    }
  }
  .header-1 {
    .w {
      vertical-align: sub;
      padding-bottom: 2px;
      border-bottom: 1px solid #d2d2d2;
      position: relative;
      .tabs {
        position: absolute;
        bottom: 0px;
        left: 300px;
        display: flex;
        flex-direction: row;
        justify-content: space-between;
        background-color: #fff;
        // width: 300px;
        li {
          color: #909399;
          line-height: 60px;
          font-size: 14px;
          padding: 0 20px;
          border-bottom: 2px solid transparent;
          a {
            box-sizing: border-box;
            display: inline-block;
            line-height: 60px;
            padding: 0 14px;
            &.router-link-exact-active {
              color: #409eff;
              border-bottom: 2px solid #409eff;
            }
          }
        }
      }
    }
  }
  footer {
    background-color: #2f2f2f;
    .footer_top {
      width: 100%;
      border-bottom: 1px solid #3d3d3d;
      .w {
        ul {
          width: 100%;
          display: flex;
          flex-direction: row;
          justify-content: space-between;
          li {
            width: 33.3%;
            text-align: center;
            a {
              box-sizing: border-box;
              display: inline-block;
              width: 195.73px;
              margin: 50px auto;
              color: #fff;
              font-size: 20px;
              padding-left: 44px;
              height: 40px;
              display: inline-block;
              line-height: 40px;
              text-decoration: none;
              background: url(data:image/png;base64,)
                no-repeat left 0;
            }
          }
        }
      }
    }
    .footer_bottom {
      width: 100%;
      padding-bottom: 18px;
      .icon {
        width: 50px;
        height: 50px;
        margin: 20px auto;
        background: url(data:image/png;base64,)
          no-repeat;
      }
      .footer_tabs {
        text-align: center;
        color: #888888;
        a {
          &.router-link-exact-active {
            color: #fff;
          }
        }
        span {
          margin: 0 22px;
        }
      }
      .footer_text {
        margin: 20px 0;
        text-align: center;
        color: #888888;
      }
    }
  }
  .ipt {
    // text-align: center;
    margin-bottom: 20px;
    padding: 0 auto;
    span {
      margin: 0 auto;
    }
  }
}
</style>